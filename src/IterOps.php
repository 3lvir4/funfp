...
interface IterOps
{
    ...
    /**
     * Creates a new IterOps instance that yields a slice of elements from this instance,
     * starting from the $start index and containing $amount elements.
     *
     * If $amount equals -1, it takes all the elements after start.
     *
     * Example:
     * ```
     * $items = iter(['a', 'b', 'c', 'd', 'e'])->unwrap();
     * $sliced = $items->slice(1, 3);
     * // $sliced->toList() results in ['b', 'c', 'd']
     * ```
     *
     * @param int $start
     * @param int $amount must be greater than or equal to -1.
     * @return IterOps<TKey, TVal>
     */
    public function slice(int $start, int $amount = -1): IterOps;

    /**
     * Creates a new IterOps instance that yields elements of this instance interspersed with a separator.
     *
     * Example:
     * ```
     * $items = iter(['a', 'b', 'c'])->unwrap();
     * $interspersed = $items->intersperse('-');
     * // $interspersed->toList() results in ['a', '-', 'b', '-', 'c']
     * ```
     *
     * @param TVal $sep
     * @return IterOps<TKey, TVal>
     ...
     * ```
     * $items = iter(['a', 'b', 'c'])->unwrap();
     * $interspersed = $items->intersperseWith(fn() => '-');
     * // $interspersed->toList() results in ['a', '-', 'b', '-', 'c']
     * ```
     *
